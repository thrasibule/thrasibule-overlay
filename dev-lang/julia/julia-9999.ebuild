...
src_prepare() {
    ...
    if use !builtin; then
        ...
        mkdir -p external/root/lib || die "Can't creat external/root/lib folder."
        ln -s /$(get_libdir)/libpcre.so.0 external/root/lib/libpcre.so || die "Can't add symbolic link to pcre"
...
src_compile() {
    cd deps || die "Could not enter 'external' directory!"
    ...
    gfortran -shared ${FFLAGS} \
        ...
        ${LIBLAPACK} ${LIBBLAS} -lstdc++ -o ${S}/external/root/lib/libsuitesparse.so
