...
src_configure() {
	...
	use szip && myconf="${myconf} --with-libisz="${stdpath}""
...
src_compile() {
	...
	emake -j1 just-hdf5|| die "emake failed"
	...
	if use hdf; then
		...
		emake -j1 just-hdf4|| die "emake failed"
	fi

	if use examples; then
		emake -j1 do-examples|| die "emake failed"
	fi

	if use doc; then
		emake -j1 javadocs ||die "docs generation failed"
	fi
}
